...
var $projectsList = (0, _helper.$)('.projects-banner');

function filter(repositories) {
	var chosen = ['atom-emoji-syntax', 'frontbook', 'papyrus'];
	return repositories.filter(function (repo) {
		return chosen.indexOf(repo.name.toLowerCase()) >= 0;
	});
}
...
function updateDOM(repositories) {
	repositories.forEach(function (repo) {
		...
		var shortened = name.replace(/atom/, '').replace(/-/g, '').toLowerCase();
		var $starsEl = $projectsList.querySelector('.project-' + shortened + ' .project-stars');
...
function addGitHubStats() {
	fetch('https://api.github.com/users/morkro/repos').then(function (data) {
		return data.json();
	}).then(filter).then(prepare).then(updateDOM).catch(console.error);
}
